...
filename = 'main_CMDA_modules_anivid_aux.m';
...
for i=18:nr
    ...
    [kh_up_xi,kh_up_zeta,kh_down_xi,kh_down_zeta] = ...
        two_keyholes_dxi(k, h, D, R, U_nd, theta, phi, m,0,DU, 2,...
        radius_factor,longp,ap,cons,kepE_sma,cons_sec);
...
%     arcexist = sum( kh_down_zeta < -1.6*RE_au );
    ...
    try
    [kh_up_xi,kh_up_zeta,kh_down_xi,kh_down_zeta] = ...
        two_keyholes_dxi(k, h, D, R, U_nd, theta, phi, m,0,DU, 3,...
        radius_factor,longp,ap,cons,kepE_sma,cons_ode);
    catch
        fprintf('Se intento bruh\n')
        continue
    end

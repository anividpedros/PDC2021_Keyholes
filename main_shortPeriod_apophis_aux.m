...
filename = 'main_shortPeriod_apophis_aux.m';
...
for i=1%:length(xiv)
    for j=1%:length(zev)
        ...
        xi0   =  150*cons.Re; %kh_down_xi(ik(1));
...
% profile viewer
% save('Apophis-Bplanemats-25Re-post.mat', 'xiv','zev','amplitude','perc_inside','max_error','dMOID_sec','min_sec','min_nbp')
...
%% -------------------------------------------
figure(50)
contourf( xiv/sc, zev/sc, min_nbp'/sc );
grid on;
hold on;
xlabel('\xi (Re)')
ylabel('\zeta (Re)')

...
for i = 1:length(tv)
    ...
    [~, kep0_LL_t(i,:)] = drifted_oe_s2( secular_model_LL, tv(i), OE0, kepJ_sma' );
    kept(2:6) = kep0_LL_t(i,2:6); 
    % kept(1)   = kep0_sma(1)*(1-kep0_LL_t(i,2));
    kept(1)   = kep0_sma(1)*(1-kep0_sma(2));
    
    xa   = cspice_conics(kept',    eti + tv(i) );
    % xa = coordinates_AnAstro( 0, kept, 0, tv(1)+eti, 1, cons.GMs);
    xe   = cspice_conics(kep_eat,  eti + tv(i) );
...
tol = 1e-13;
options=odeset('RelTol',tol,'AbsTol',ones(1,6)*tol);
[t,EQt]=ode113(@(t,X) ShortPeriod_prop_dev(t,X,cons_aux,secular_model_LL),tv,EQ0,options);
...
%     MOIDsp(i) = MOID_ORCCA_win( K2S(kepE_sma,cons.AU), K2S(kep_sp(i,:),cons.AU) ) *cons.AU;
    MOIDsp(i) = MOID_SDG_win( kep_sp(i,[1 2 4 3 5]), kepE_sma([1 2 4 3 5]) );
    
    % Compute distance to Earth
    xe   = cspice_conics(kep_eat, eti+tv(i) );
    xsp  = coordinates_AnAstro( 0, kep_sp(i,:), 0,tv(i) , 1,cons.GMs);
    d_sp(i) = norm(xe(1:3) - xsp(1:3));
end

%
% See if results make some sense
figure(50); clf;
for i=1:6
    subplot(2,3,i)
%     plot(t/xsc,EQt(:,i))
...
%
figure(5)
plot(tv/xsc, MOIDsp/cons.Re, 'c')

figure(6)
plot(tv/xsc, d_sp/cons.Re, 'c')



%% Short-period component integration validation
% Checklist:
% F- Without secular should be = to num. integration
%    - Finite differences for dKep_dSemieq
% X- Integration of LPE should match analytical LL
%  NA- Finite differences for dsec_dt

% Re-integrate ------------------------
OE0    = kep0_sma(1:6) ;
% OE0(6) = OE0(7) - OE0(6)/sqrt(OE0(8)/OE0(1)^3) ;
% This is tau! We need sigma
OE0(6) = OE0(6) - sqrt(cons.GMs/OE0(1)^3)*kep0_sma(7) ;
...
% OEJ(6) = OEJ(7) - OEJ(6)/sqrt(OEJ(8)/OEJ(1)^3) ;
% This is tau! We need sigma
OEJ(6) = OEJ(6) - sqrt(cons.GMs/OEJ(1)^3)*kepJ_sma(7) ;


EQ0 = Kep_2_Equinoctial( OE0 );
EQ0(1) = a2L( EQ0(1),cons.GMs );
...
secular_model_LL = secular_model_10BP_extra(kep0_sma, cons_sec, 1);
Xdot = ShortPeriod_prop_dev(0, EQ0, cons_aux, secular_model_LL);
...
for i = 1:length(t)
    kep_sp(i,:) = Equi_2_Keplerian(EQt(i,:));
    kep_sp(i,1) = L2a( kep_sp(i,1),cons.GMs );
end
%---------------------------------------
% See if results make some sense
figure(50); clf;
...
kep_nbp_sig = kep_nbp;
kep_nbp_sig(:,6) = wrapTo2Pi(kep_nbp_sig(:,6) - sqrt(cons.GMs./kep_nbp_sig(:,1).^3).*(eti+tv(:))); 

kep0_LL_t(:,4) = wrapTo2Pi(kep0_LL_t(:,4));
kep0_LL_t(:,6) = kep0_LL_t(:,6) + kep_nbp_sig(1,6)-kep0_LL_t(1,6);

kep_sp(:,4) = wrapTo2Pi(kep_sp(:,4));
kep_sp(:,6) = wrapTo2Pi(kep_sp(:,6));
...
%     plot(t/xsc,kep_sp(:,i))
    ...
    plot(tv/xsc, kep_nbp_sig(:,i))
    hold on
    plot(tv/xsc, kep0_LL_t(:,i))
    plot(tv/xsc, kep_sp(:,i))
    grid on
end
% kep0_LL_t(i,:)
% kep_nbp
...
%% Does sigma definition work?
clc
OE0 = kep0_sma(1:6);
OE0(6) = OE0(6) - sqrt( cons.GMs/OE0(1)^3 )*eti ;

X_n = coordinates_AnAstro( 0, OE0, 0, tv(1)+eti, 1, cons.GMs)
xa  = cspice_conics(   kep_opik_post, tv(1)+et0 )
...
OE1 = Equi_2_Keplerian( EQ1 );

% OE0(1) = a2L( OE0(1),cons.GMs );
% OE1(1) = a2L( OE1(1),cons.GMs );

dOE_dseq = dKep_dSEqui(EQ0,OE0, cons)

dOE_dseq_FD = (OE1 - OE0(1:6))/tol

...
load('Horizons-1999AN10-Earth.mat')
...
%     state_eat = cspice_spkezr( '399', et, 'ECLIPJ2000', 'NONE', '10' );
    state_eat = [Xe(i); Ye(i); Ze(i); VXe(i); VYe(i); VZe(i)];
...
figure(12);
subplot(2,1,1)
plot( JDTDB - JDTDB(1), thv*180/pi ); ylabel('\theta')
grid on; hold on
subplot(2,1,2)
plot( JDTDB - JDTDB(1), phv*180/pi ); ylabel('\phi')
grid on; hold on


figure(13)
subplot(2,1,1)
plot( JDTDB-JDTDB(1), V/(DU/TU) )
ylabel('V ND')
subplot(2,1,2)
plot( JDTDB-JDTDB(1), rt/cons.AU )
ylabel('d (au)')
...
filename = 'main_shortPeriod_apophis.m';
...
final_moid  = nan(length(xiv),length(zev));
max_nbp = final_moid;
...
%         zeta0 =  20*cons.Re; %kh_down_zeta(ik(1));
        ...
        % Accumulate metrics
        i1 = find(tv >= 50*365.25*24*3600, 1);
        perc_inside(i,j) = 100*sum( abs(MOIDnbp(1:i1) - MOIDsec(1:i1))< mean_std )/length(tv(1:i1));
        ...
        final_moid(i,j)  = MOIDnbp(end);
        max_nbp(i,j)     = max(MOIDnbp);
        
        % fprintf('[i=%g/%g,j=%g/%g] - b={%g,%g}Re -  perc=%.1f\n',i,length(xiv),j,length(zev),xi0/cons.Re,zeta0/cons.Re,perc_inside(i,j))
        fprintf(' perc=%.1f - max_nbp=%g - final_nbp=%g\n',perc_inside(i,j),max_nbp(i,j)/cons.Re,final_moid(i,j)/cons.Re)
        
    end
end

% profile viewer
% save('2006-Bplanemats-25Re-post.mat', 'xiv','zev','amplitude','perc_inside','max_error','dMOID_sec','min_sec','min_nbp','max_nbp','final_moid')
...
axis equal
F.Position([3 4]) = [428 300];

%% -------------------------------------------
F = figure(45); clf; 

contourf( xiv/sc, zev/sc, perc_inside' );
grid on;
hold on;
xlabel('\xi (Re)')
ylabel('\zeta (Re)')
...
contourf( xiv/sc, zev/sc, max_error'/sc );
grid on;
hold on;
xlabel('\xi (Re)')
ylabel('\zeta (Re)')
...
contourf( xiv/sc, zev/sc, amplitude'/sc/1.5, 20, 'LineStyle', 'none' );
hold on
contour( xiv/sc, zev/sc, amplitude'/sc/1.5, [0:2:10],'k', 'ShowText', 'on' );
...
%% Add semi-analytical propagation!

% Substitute running the script with the relevant lines
% run('initialize_secssMCs_simp')
% run('initialize_wmat')
%------------------------------------
load('C:\Users\Oscar\odrive\Google Drive\Code2022\FuentesMunoz2022 - Scripts and Data\constants.mat')
addpath('C:\Users\Oscar\odrive\Google Drive\Code2022\FuentesMunoz2022 - Scripts and Data\')
tsim    = [0 1e2] ;
enc_tol = 0.10 ;
integration_time= 0.1 ;

sim_setup.TimeArrow = 1;

% Multiencounter module setup
Tmax = tsim(end);
Trec = .2;

% 1.2. Modules defintions
ilim    = 80 *pi/180 ;
elim    = 0.01 ;
OE_lim  = [5 0.9 ilim];
t0      = 0;
...
sim_setup.Tmax = Tmax;
sim_setup.Trec = Trec;
sim_setup.OElim = OE_lim;

sim_setup.t0   = t0 ;
sim_setup.OEp  = cons.OEp ;
sim_setup.GMp  = cons.GMp ;

sim_setup.ilim = ilim;
sim_setup.elim = elim;
sim_setup.enc_tol = enc_tol;
sim_setup.int_time = integration_time;
%------------------------------------

cons.GMe = cons.GMp(3); % Add elements that cannot be added inside mex
cons.time_int = 0;      % Add elements that cannot be added inside mex
mcNameFile = ['ER/' sprintf('%.4d',1) '.csv'];
    
% MC Runs
sim_setup.writeFile = 0;

% Individual Simulation Setup
% OE0_SIG = kep0_sma(1:6);
% OE0_SIG(1) = OE0_SIG(1)/cons.AU;

OE0_SIG = kep_opik_post(1:6)';
OE0_SIG(1) = OE0_SIG(1)/(1-OE0_SIG(2)) /cons.AU;

% B. Multiencounters
Res1 = multiencounter_sec6_mex( sim_setup, cons, mcNameFile, OE0_SIG );
